...
void Battery_info_polling_Process(unsigned char bms_index)
{
    ...
    if(NFC_READER_COUNT_MAX <= bms_index)
    {
        return ;
    }
    ...
    //如果是正在升级，也退出
    if((gl_bms_update_state[gl_modbus_param.BMS_index] == ENUM_BMS_UPDATE_XMODEM_PROCESS)&&
        (gl_xmodem_state[gl_modbus_param.BMS_index] != ENUM_XMODEM_IDLE))
    {
        return ;
    }
    ...
    //如果正在与电池充电，也退出
    if(gl_bms_info_p[bms_index]->waiting_cmd_ack)
        return ;
