...
void Battery_discharge_process(void)
{
    ...
    if((main_is_cmd_poweroff_valid())//停车执行
...
//            (0 != (gl_bms_info_p[0]->reg_unit[vl_state_pos].bits16_L & (1<<3)))&& // 判断霍尔开关
//            ((0 != (gl_bms_info_p[0]->reg_unit[vl_state_pos].bits16_L & (1<<0)))|| // 判断是否关闭充电
//            //(0 == (gl_bms_info_p[0]->reg_unit[vl_pos].bits16_L&(1<<5))))&& // 判断是否处于休眠模式
//            //(0 == is_battery_error(0)))
//            (0 == (gl_bms_info_p[0]->reg_unit[vl_pos].bits16_L&(1<<5)))|| // 判断是否处于休眠模式
//            ((0 == (gl_bms_info_p[0]->reg_unit[vl_pos].bits16_L&(1<<6)))&& //需要进入深度睡眠
...
//            (0 != (gl_bms_info_p[1]->reg_unit[vl_state_pos].bits16_L&(1<<3)))&& // 判断霍尔开关
//            ((0 != (gl_bms_info_p[1]->reg_unit[vl_state_pos].bits16_L&(1<<0)))|| // 判断是否关闭充电
//            //(0 == (gl_bms_info_p[1]->reg_unit[vl_pos].bits16_L&(1<<5))))&&
//            //(0 == is_battery_error(1)))
//            (0 == (gl_bms_info_p[1]->reg_unit[vl_pos].bits16_L&(1<<5)))||
//            ((0 == (gl_bms_info_p[1]->reg_unit[vl_pos].bits16_L&(1<<6)))&& //需要进入深度睡眠
//            (0 == Battery_get_PMS_working_state())&&
//            (main_get_have_been_ACC_ON_flag()))))
//        {
//            vl_force_to_exe[1] = 1;
...
//        Battery_clear_high_voltage_bat_err_param(0);
        ...
        #ifdef PMS_SELFOFF_TIME_COUNTER_ENABLE
        if((vl_bms_0_A) < (PMS_SELFOFF_CURRENT_MAX))// 输出电流大于2500mA
        {
        ...
        #ifdef PMS_CMD_POWEROFF_TIME_COUNTER_ENABLE
        // 输出电流 大于2.5A，充电电流大于2A，认为是在行车状态
        if(((vl_bms_0_A) < (main_cmd_poweroff_get_A_out_value_max()))// 输出电流大于2500mA
            || ((vl_bms_0_A) > (200))) //大于2A的充电
        {
            ...
            else if((main_get_bicycle_stopped_status_for_smart()) && //停车中
...
//        }
        ...
        //打开电池0的充放电开关
            ...
            ((BMS_STATE_BIT_SUPPLY_SWITCH&vl_bms_0_ctrl) == 0))||
                ((!sl_battery_need2sleep_flag)&&(0 != (gl_bms_info_p[0]->reg_unit[vl_pos].bits16_L&(1<<5)))))//关闭睡眠模式
        {
...
//        }
//        // ACC OFF 情况下， ACC OUT输出off; 2018-6-29
...
//        
//        #ifdef PMS_SELFOFF_TIME_COUNTER_ENABLE
//        if((vl_bms_0_A+vl_bms_1_A) < (PMS_SELFOFF_CURRENT_MAX))// 输出电流大于2500mA
//        {
//            main_pms_selfoff_detect_bat_current_out();
//        }
//        #endif
...
//        #ifdef PMS_CMD_POWEROFF_TIME_COUNTER_ENABLE
//        // 输出电流 大于2.5A，充电电流大于2A，认为是在行车状态
//        if(((vl_bms_0_A+vl_bms_1_A) < (main_cmd_poweroff_get_A_out_value_max()))// 输出电流大于2500mA
//            || ((vl_bms_0_A+vl_bms_1_A) > (200))) //大于2A的充电
//        {
//            if(main_cmd_poweroff_get_poweroff_flag_nonvolatilely())
//            {
//                //这时候立刻断电
...
//                    ((!sl_battery_need2sleep_flag)&&(0 != (gl_bms_info_p[1]->reg_unit[vl_pos].bits16_L&(1<<5)))))//关闭睡眠模式
//                {
//                    if(gl_illegal_charging_in_flag)
//                    {
...
//        }
//        else if(2 == sl_supply_state)//if (gl_Battery_delay_process_cnt) //gl_Battery_delay_process_cnt延迟后，不能再操作
//        {
//             
//            //两电池压差大于0.5V
...
//                    {
//                        //低电压电池的放电开关或者充电开关是关闭的，这时要求都打开打开
//                        if(gl_illegal_charging_in_flag)
//                        {
//                            Bttery_check_pre_error_and_set_switch_state(1, BMS_STATE_BIT_SUPPLY_SWITCH);
//                        }
...
//        #ifdef PMS_SELFOFF_TIME_COUNTER_ENABLE
//        if((vl_bms_1_A) < (PMS_SELFOFF_CURRENT_MAX))// 输出电流大于2500mA
//        {
//            main_pms_selfoff_detect_bat_current_out();
//        }
//        #endif
...
//        #ifdef PMS_CMD_POWEROFF_TIME_COUNTER_ENABLE
//        // 输出电流 大于2.5A，充电电流大于2A，认为是在行车状态
//        if(((vl_bms_1_A) < (main_cmd_poweroff_get_A_out_value_max()))// 输出电流大于2500mA
//            || ((vl_bms_1_A) > (200))) //大于2A的充电
//        {
//            if(main_cmd_poweroff_get_poweroff_flag_nonvolatilely())
//            {
//                //这时候立刻断电
...
void Battery_set_switch_state(unsigned char bms_index, unsigned short on)
{
    ...
    //判断ACC 状态，是去抖后的状态; 判断是否允许放电
    //if ((0 == gl_acc_wake_status)||(0 == Battery_get_PMS_working_state()))
    //if ((sl_wake1_update_flag == 0)&&(0 == Battery_get_PMS_working_state())) //升级时候不能拦截
    //    return ;
