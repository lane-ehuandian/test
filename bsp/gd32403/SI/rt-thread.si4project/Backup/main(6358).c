...
/*全局变量*/
unsigned int gl_delay_cnt_test = 0;
...
typedef struct
{
    ...
    /*debug*/
    struct rt_thread thread_debug;
	unsigned char thread_debug_stack[1024]; 
	/*fm17522*/
...
//    rt_thread_startup(&sl_main_param.thread_led);
	...
	//debug
	res=rt_thread_init(&sl_main_param.thread_debug,
    "debug",
    thread_debug_entry,
    RT_NULL,
    &sl_main_param.thread_debug_stack[0],
    sizeof(sl_main_param.thread_debug_stack),
    8, 10);	
    if (res == RT_EOK) /* 如果获得线程控制块，启动这个线程 */
    {
        rt_thread_startup(&sl_main_param.thread_debug);
    }
	else
	{
