...
/*
*i2c_flag_get_via_reg_value; Added by Kim
*/
//FlagStatus i2c_flag_get_via_reg_value(uint32_t state0_reg, uint32_t state1_reg, uint32_t flag)
//{
//    uint32_t reg = 0U;
...
//    reg = (BIT(31) & flag);
//    if((BIT(31) == reg)){
//        if((state1_reg & (flag & I2C_FLAG_MASK))){
//            reval = SET;
//        }else{
//            reval = RESET;
//        }
//    }else{
//        if((state0_reg & (flag & I2C_FLAG_MASK))){
//            reval = SET;
//        }else{
//            reval = RESET;
//        }
...
void I2C1_EventIRQ_Handler(void)
{
    ...
    if(i2c_flag_get(bus_select, I2C_FLAG_SBSEND))
    {
        //vl_gd32_bus = gd32_i2c_get_bus_param(bus_select);
        /* send slave address to I2C bus */
        i2c_master_addressing(bus_select, vl_gd32_bus->i2c_slave_addr, vl_gd32_bus->i2c_tran_dir);
    }
    else if(i2c_flag_get(bus_select, I2C_FLAG_ADDSEND))
    {
        ...
        if(I2C_RECEIVER == vl_gd32_bus->i2c_tran_dir)
        {
            if(1 == vl_gd32_bus->i2c_data_buf_len)
            {
                ...
                if(i2c_flag_get(bus_select, I2C_FLAG_MASTER))
                {
    ...
    else if((I2C_TRANSMITTER == vl_gd32_bus->i2c_tran_dir)&&(i2c_flag_get(bus_select, I2C_FLAG_TBE)))
    {
        //vl_gd32_bus = gd32_i2c_get_bus_param(bus_select);
        if(0 == vl_gd32_bus->i2c_data_buf_len)
        {
            if((0 == (I2C_CTL0(bus_select)&0x0200))/*是否已经置位stop位*/
                &&(i2c_flag_get(bus_select, I2C_FLAG_I2CBSY)))/*是否i2c通讯忙*/
            {
                if(i2c_flag_get(bus_select, I2C_FLAG_MASTER))
                {
                    /* send a stop condition to I2C bus */
    ...
    else if((I2C_TRANSMITTER == vl_gd32_bus->i2c_tran_dir)&&(i2c_flag_get(bus_select, I2C_FLAG_BTC)))
    {
        if(0 == vl_gd32_bus->i2c_data_buf_len)
        {
            if((0 == (I2C_CTL0(bus_select)&0x0200))/*是否已经置位stop位*/
    ...
    else if(i2c_flag_get(bus_select, I2C_FLAG_RBNE))
    {
        //vl_gd32_bus = gd32_i2c_get_bus_param(bus_select);
...
{    
    gd32_bus->i2c_sem = rt_sem_create(sem_name, 1, RT_IPC_FLAG_PRIO);
